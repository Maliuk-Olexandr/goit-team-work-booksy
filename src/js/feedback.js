document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateDesktopOnlySlides() {
      if (window.innerWidth >= 1440) {
        // На больших экранах показываем .feedbacks-desktop-only и даем класс swiper-slide
        desktopOnlyItems.forEach(item => {
          item.style.display = 'block';  // или 'block', если нужно
          item.classList.add('swiper-slide');
        });
      } else {
        // На меньших экранах скрываем и убираем класс swiper-slide, чтобы Swiper не учитывал их
        desktopOnlyItems.forEach(item => {
    ...
    window.addEventListener('resize', () => {
      updateDesktopOnlySlides();
      feedbacksSwiper.update(); // обновляем Swiper, чтобы учел изменения классов и элементов
    });
